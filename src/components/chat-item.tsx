...
const ChatItem = ({
  ...
  const isRegulator = channelData?.regulators?.includes(currentUser.id);
  const isOwner = currentUser.id === user.id;
  const canDeleteMessage = !deleted && (isOwner || isSuperAdmin || isRegulator);
  const canEditMessage = !deleted && isOwner && !fileUrl;
  const isPdf = fileType === 'pdf' && fileUrl;
  const isImage = fileType === 'img' && fileUrl;
    ...
    <>
      {isImage && (
        <Link
          ...
          className="relative aspect-square rounded-md mt-2 overflow-hidden border flex item-center bg-secondary h-28 w-48"
        >
          <Image
            src={publicUrl}
            alt={content ?? ''}
      ...
      {isPdf && (
        <div className="flex flex-col items-start justify-center gap-2 px-2 py-1 border rounted-md shadow bg-white dark:bg-gray-800">
          <Typography
            variant="p"
            text="shated a file"
  ...
  const EditableContent = () =>
    isEditing ? (
      ...
      <div
        className={cn('text-sm', { 'text-sx opacity-90 italic': deleted })}
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <div className="text-center">
            {isPdf && (
              <div className="flex flex-col items-start justify-center gap-2 px-2 py-1 border rounted-md shadow bg-white dark:bg-gray-800">
                <Typography variant="p" text="shated a file" />
                <Link
                  href={publicUrl}
                  target="_blank"
                  rel="noopener noreferrre"
  ...
  return (
    <div className="relative group flex items-center hover:bg-black/5 px-1 py-2 rounded transition w-full">
      <div className="flex gap-x-2 ">
        <div className="cursor-pointer hover:drop-shadow-sm-md transision">
          <Avatar>
            <AvatarImage
              src={user.avatar_url}
